...
router.get('/:id/contacts', auth, wrap(async(req, res) => {
  res.send(await getContactsForUser(req.params.id));
}));

router.post('/:id/contacts', auth, wrap(async(req, res) => {
  const contact = await createContact(req.params.id, req.body);
  res.status(201);
  res.send(contact);
}));

router.get('/:id/contacts/:contactId', auth, wrap(async(req, res) => {
  res.send(await getSingleContact(req.params.contactId));
}));

router.put('/:id/contacts/:contactId', auth, wrap(async(req, res) => {
  await updateContact(req.params.contactId, req.body);
  res.sendStatus(204);
}));

router.delete('/:id/contacts/:contactId', auth, wrap(async(req, res) => {
